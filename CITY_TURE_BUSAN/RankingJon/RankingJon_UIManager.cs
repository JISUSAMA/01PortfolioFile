...
public class RankingJon_UIManager : MonoBehaviour {
    ...
    string[] mapNameStr = new string[4];    //판넬 이름들
    ...
    private void UpdateString() {
        ...
        int currentLocale = LanguageSelectorManager.instance.GetCurrentSelectedLanguage();

        if (currentLocale == (int)LanguageType.English) {
            mapNameStr[0] = "Busan Normal Course 1";
            mapNameStr[1] = "Busan Normal Course 2";
            mapNameStr[2] = "Busan Hard Course 1";
            mapNameStr[3] = "Busan Hard Course 2";
    ...
    string Input_RankingValue(int _rank, string _name) {
        switch (_name) {
            ...
            case "CorseText":
                ...
                if (currentLocale == (int)LanguageType.English) {
                    return "Course " + ServerManager.Instance.Ranking[_rank].corse;
                } else if (currentLocale == (int)LanguageType.Korean) {
    ...
    public void FilterSearchButtonOn() {
        ...
        if (modeDrop.value == (int)ModeType.Normal) {
            if (corseDrop.value == (int)CourseType.Course1) {
                RankingCorsePanel_ActionShow(true, false, false, false);//, false, false);
                panelNameText.text = mapNameStr[0]; //노말 제 1코스
            } else if (corseDrop.value == (int)CourseType.Course2) {
                RankingCorsePanel_ActionShow(false, true, false, false);//, false, false);
                panelNameText.text = mapNameStr[1]; //노말 제 2코스
            }
        } else if (modeDrop.value == (int)ModeType.Hard) {
            if (corseDrop.value == (int)CourseType.Course1) {
                RankingCorsePanel_ActionShow(false, false, true, false);//, false, false);
