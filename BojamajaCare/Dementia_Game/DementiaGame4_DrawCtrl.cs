...
public class DementiaGame4_DrawCtrl : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!DementiaGame4_DataManager.instance.playBool) Destroy(this.gameObject);
    }
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (col_List.Count >= 2)
        {
            ...
            {
                if (DementiaGame4_DataManager.instance.AnswerQuestionCount < 3)
                {
                ...
                {
                    DementiaGame4_DataManager.instance.Question_Success();
                }

            }
            else
            {
                DementiaGame4_DataManager.instance.Question_Fail();
                this.gameObject.GetComponent<LineRenderer>().positionCount = 0;
            }
        }
