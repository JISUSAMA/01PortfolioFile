...
public class DementiaGame2_DrawCtrl : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!DementiaGame2_DataManager.instance.playBool) Destroy(this.gameObject);
    }
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (col_List.Count >= 2)
        {
            ...
            if (start_proverbNum.Equals(end_proverbNum))
            {
                ...
                {
                    DementiaGame2_DataManager.instance.Question_Success();
                }

            }
            else
            {
                DementiaGame2_DataManager.instance.Question_Fail();
                this.gameObject.GetComponent<LineRenderer>().positionCount = 0;
            }
        }
