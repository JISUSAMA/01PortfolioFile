...
public class DementiaGame3_DrawCtrl : MonoBehaviour
{
    ...
    private void Update()
    {
       ...
       if (!DementiaGame3_DataManager.instance.playBool) Destroy(this.gameObject);
    }
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (spstring[0].Equals("start"))
        {
            ...
            string obj_str = DementiaGame3_DataManager.instance.Object_Data_SuffleNum[star_num]; //리스트 x번째 있는 속담
        ...
        else if (spstring[0].Equals("end"))
        {
            ...
            string objNum_str = DementiaGame3_DataManager.instance.Object_Data_Num[end_num];
        ...
        if (col_List.Count >= 2)
        {
            ...
            if (start_proverbNum.Equals(end_proverbNum))
            {
                ...
                {
                    DementiaGame3_DataManager.instance.Question_Success();
                }

            }
            else
            {
                DementiaGame3_DataManager.instance.Question_Fail();
                this.gameObject.GetComponent<LineRenderer>().positionCount = 0;
            }
        }
