...
public class ToyPart : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        //    Player.CheckWhetherToRelease(this); 
            Debug.Log("출돌 하는 이름 : " + col.gameObject);
            if (col.gameObject.layer == LayerMask.NameToLayer("Buildings") && this.Active.Equals(true))
            {
                Debug.Log("------------------------1");
                ...
                Debug.Log("------------------------2");
                ToyFactory.PlayerController.Instance.holdBool = false;
                Active = false;
                Destroy(this.gameObject.transform.gameObject);
            }
            if (col.gameObject.layer == LayerMask.NameToLayer("Buildings") && this.Active.Equals(false))
            {
                Destroy(this.gameObject.transform.gameObject);
            }
            //layer : wall
