...
public class OrderManager : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider collider)
    {
      ...
      /*  Rigidbody colRig = col.GetComponent<Rigidbody>();
        colRig.useGravity = true; */
     
        FoodItem food = col.GetComponent<FoodItem>(); //음식 재료 
    ...
    public void EvaluateIngredient(FoodItem foodItem)
    {
        ...
        if (foodItem.Equals(_currentSandwich.recipe[_recipeStep]))
        {
            ...
            _recipeStep++;
            DataManager.Instance.scoreManager.Add(150); //100점 획득
            ...
            if (_recipeStep > _currentSandwich.recipe.Length - 1) // If we finish the current sandwich
            {
                ...
                DataManager.Instance.scoreManager.Add(800); //1000점 획득
            }
        }
        //재료를 잘못 놓았을 떄
        else
        {
            Rigidbody Food = foodItem.GetComponent<Rigidbody>();
            Food.useGravity = true;
        ...
        StartCoroutine(_OtherObCanTouch());
    }
    IEnumerator _OtherObCanTouch()
    {
        yield return new WaitForSeconds(1f);
        Sandwiches.PlayerController.CantTouch = false;
        yield return null;
    }
    //재료를 완성 했을 떄
