 ...
 /*   public ChanceScore[] stageNum;
    ...
    IEnumerator _ChanceSetScore()
    {
        for (int i = 0; i < stageNum.Length; i++)
        {
            ...
            {
                stageNum[i].ScoreNUM = PlayerPrefs.GetFloat("MonseterBird_Score");
                stageNum[i].GameName = "괴물새를 쫓아라";
                stageNum[i].GameSceneName = "MonseterBird";
            }
            ...
            {
                stageNum[i].ScoreNUM = PlayerPrefs.GetFloat("Statue_Score");
                stageNum[i].GameName = "조각상을 만들어라";
                stageNum[i].GameSceneName = "Statue";
            }
            ...
            {
                stageNum[i].ScoreNUM = PlayerPrefs.GetFloat("Flag_Score");
                stageNum[i].GameName = "청기백기를 올려라";
                stageNum[i].GameSceneName = "Flag";
            }
            else if(GameManager.Season2_NextCallNum[i].Equals(7))
            {
                stageNum[i].ScoreNUM = PlayerPrefs.GetFloat("Pipe_Score");
                stageNum[i].GameName = "호스를 잠가라";
                stageNum[i].GameSceneName = "Pipe";
            }
    ...
    public void ChanceTime()
    {
        ...
        if (CheckisOnToggle())
        {
            ...
            else
            {
                ...
                float score = PlayerPrefs.GetFloat("GameTotalScore");
                TimeManager.instance.diamondSu -= 2;
                PlayerPrefs.SetInt("Diamond", TimeManager.instance.diamondSu);
                PlayerPrefs.SetFloat("GameTotalScore", score - CheckGameScore);
