...
public class RunTimeGraphDrawLine : MonoBehaviour
{
    public static RunTimeGraphDrawLine instance { get; private set; }
    ...
    IEnumerator _GraphDrawLineMake()
    {
        ...
        for (int i = 0; i < graph.Length; i++)
        {
            if (parants[i].activeSelf == true)
            {
                if (graph[i].activeSelf == true)
                {
                    ...
                    //라인 포지션 위치를 객체 포지션 위치랑 동일하게 하기 위해 객체 위치를 저장
                    //float x = graph[i].transform.position.x;
                    //float y = graph[i].transform.position.y;
                    //graph[i].transform.position = new Vector3(x, y, -90f);
        ...
        {
            graphPos[i] = new Vector3(graph[graphIndex[i]].transform.position.x, graph[graphIndex[i]].transform.position.y + 2f, 95f);
