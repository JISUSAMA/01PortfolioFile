...
public class RunTimeGraphDisplay : MonoBehaviour
{
    public static RunTimeGraphDisplay instance { get; private set; }
    ...
    public bool isTimeGraphOn = false;

    [Header("RunTime_About Graphs")]
    ...
    public void MonthInitalization()
    {
        ...
        DayDataDelete();
        _GraphTypeImge("dot");
        RunTimeGraphDrawLine.instance.GraphDrawLineMake();
    }
    ...
    void DayDataDelete()
    {
        ...
        for (int i = 0; i < 31; i++)
        {
            dayData_scrip = timeView[i].gameObject.GetComponent<DayData>();
            dayData_scrip.y = 0;
            ...
            dayData_scrip.LengthExtend(); //그래프 길이 함수
        }
    }
    ...
    IEnumerator _DayDataInit(int _year, int _presentMonth, int _today, int _startDay)
    {
        ...
        Debug.Log("_startDay : " + _startDay);
        Debug.Log("_today : " + _today);

        // 실제 데이터의 최대 높이 구하기
        for (int max = 0; max < ServerManager.Instance.dataByDate.Count; max++)
        {
            sp = ServerManager.Instance.dataByDate[max].today_StepTime.Split(new[] { ":", "." }, StringSplitOptions.RemoveEmptyEntries);

            hour = int.Parse(sp[0]) * 60 * 60;
            minute = int.Parse(sp[1]) * 60;
            sec = int.Parse(sp[2]);
            totalTime = hour + minute + sec;

            if (top_heightSize < totalTime)
            {
                top_heightSize = totalTime;
                biggestValue = totalTime;
            }
        }
        ...
        string s_hours = ((int)one_third_heightSize / 3600).ToString("D2");//시 공식
        string s_minute = (((int)one_third_heightSize % 3600) / 60).ToString("D2");//분을 구하기위해서 입력되고 남은값에서 또 60을 나눈다.
        string s_second = (((int)one_third_heightSize % 3600) % 60).ToString("D2");//마지막 남은 시간에서 분을 뺀 나머지 시간을 초로 계산함

        _one_third_Size.text = s_hours + ":" + s_minute + ":" + s_second;

        s_hours = ((int)two_third_heightSize / 3600).ToString("D2");//시 공식
        s_minute = (((int)two_third_heightSize % 3600) / 60).ToString("D2");//분을 구하기위해서 입력되고 남은값에서 또 60을 나눈다.
        s_second = (((int)two_third_heightSize % 3600) % 60).ToString("D2");//마지막 남은 시간에서 분을 뺀 나머지 시간을 초로 계산함

        _two_third_Size.text = s_hours + ":" + s_minute + ":" + s_second;

        // dayData_scrip.y 최대 600
        // 하루 : 0 ~ 24시간 => 0 ~ 600
        // 24시간을 초로 바꾸면 86400
        // 0 ~ 86400 > 0 ~ 600
        ...
        for (int i = _startDay; i < _today; i++)
        {
            ...
            if (_presentMonth == 1)
            {
                ...
                for (int j = 0; j < ServerManager.Instance.dataByDate.Count; j++)
                {
                    ...
                    // arrDate[0]년 / arrDate[1]월/ arrDate[2]일
                    if ((i + 1) == int.Parse(arrDate[2]))
                    {
                        dayData_scrip.time = ServerManager.Instance.dataByDate[j].today_StepTime;

                        time = ServerManager.Instance.dataByDate[j].today_StepTime.Split(new[] { ":", "." }, StringSplitOptions.RemoveEmptyEntries);
                        // 시간 초로 바꾸고 InversLerp 로 비율 바꾸기
                        convertToSec = int.Parse(time[2]) + (int.Parse(time[1]) * 60) + (int.Parse(time[0]) * 60 * 60);

                        dayData_scrip.y = convertToSec;
            ...
            else if (_presentMonth == 2)
            {
                ...
                for (int j = 0; j < ServerManager.Instance.dataByDate.Count; j++)
                {
                    ...
                    if ((i + 1) == int.Parse(arrDate[2]))
                    {
                        ...
                        dayData_scrip.y = convertToSec; //서버에서 값을 들고오세욧1!!!! 일별 시간!! 배열로 들고오기!!! 
                        dayData_scrip.posY = 0f;
                        dayData_scrip.LengthExtend();
                    }
                }
            ...
            else if (_presentMonth == 5)
            {
                ...
                for (int j = 0; j < ServerManager.Instance.dataByDate.Count; j++)
                {
                    ...
                    if ((i + 1) == int.Parse(arrDate[2]))
                    {
                        ...
                        Debug.Log("오늘시간 : "+ dayData_scrip.time);
                        // new Char[] { '<', '>',' ' } : 문자열 여러개일 경우
                        time = ServerManager.Instance.dataByDate[j].today_StepTime.Split(new[] { ":", "." }, StringSplitOptions.RemoveEmptyEntries);
                        // 시간 초로 바꾸고 InversLerp 로 비율 바꾸기
                        convertToSec = int.Parse(time[2]) + (int.Parse(time[1]) * 60) + (int.Parse(time[0]) * 60 * 60);

                        Debug.Log("convertToSec : " + convertToSec);
    ...
    //일별 그래프 생성해주는 함수
    public void DayTextSetActionInit(int _firstYear, int _firstMonth, int _today, int _lastday, int _fristDay)
    {
        Debug.Log(_today);
    ...
    IEnumerator _MonthDataInit(int _year, int _startMonth, int _endMonth)
    {
        ...
        for (int i = _startMonth; i < _endMonth; i++)
        {
            for (int j = 0; j < ServerManager.Instance.dataByMonth.Count; j++)
            {
                ...
                {
                    time = ServerManager.Instance.dataByMonth[j].month_StepTime.Split(new[] { ":", "." }, StringSplitOptions.RemoveEmptyEntries);
                    ...
                    {
                        if (top_heightSize < convertToSec)
                        {
                            top_heightSize = convertToSec;
                            biggestValue = convertToSec;
                        }
                    }
                    top_heightSize += top_heightSize / 10;
                    one_third_heightSize = Mathf.Floor((biggestValue / 3));
                    ...
                    //dayData_scrip.y = 50;
                    dayData_scrip.time = ServerManager.Instance.dataByMonth[j].month_StepTime;
                    dayData_scrip.posY = Mathf.Lerp(0, 632, Mathf.InverseLerp(0, top_heightSize, convertToSec));

                    Debug.Log("시간 Y : " + dayData_scrip.y);
                    Debug.Log("시간 posY : " + dayData_scrip.posY);
    ...
    public void MonthTextSetActionInit(int _startMonth, int _endMonth)
    {
        ...
        {
            timeView[i].SetActive(true);
            //  timeView[i].gameObject.GetComponent<DayData>().DotImg_Change();
