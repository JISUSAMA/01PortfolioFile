...
public class RunStepGraphDisplay : MonoBehaviour
{
    public static RunStepGraphDisplay instance { get; private set; }
    ...
    public void MonthInitalization()
    {
        ...
        DayDataDelete();
        _GraphTypeImge("dot");
        RunStepGraphDrawLine.instance.GraphDrawLineMake();
    }
    ...
    IEnumerator _DayDataInit(int _year, int _presentMonth, int _today, int _startDay)
    {
        ...
        _one_third_Size.text = one_third_heightSize.ToString() + " Step";
        _two_third_Size.text = two_third_heightSize.ToString() + " Step";
        //데이터 넣기
        for (int i = _startDay; i < _today; i++)
        {
            ...
            if (_presentMonth == 1)
            {
                ...
                for (int j = 0; j < ServerManager.Instance.dataByDate.Count; j++)
                {
                    ...
                    {
                        dayData_scrip.y = ServerManager.Instance.dataByDate[j].today_StepCount; //서버에서 값을 들고오세욧1!!!! 일별 시간!! 배열로 들고오기!!! 
    ...
    IEnumerator _MonthDataInit(int _year, int _startMonth, int _endMonth)
    {
        ...
        {
            if (top_heightSize < ServerManager.Instance.dataByMonth[max].month_StepCount)
            {
                top_heightSize = ServerManager.Instance.dataByMonth[max].month_StepCount;
                biggestValue = ServerManager.Instance.dataByMonth[max].month_StepCount;
            }
        }
        ...
        for (int i = _startMonth; i < _endMonth; i++)
        {
            for (int j = 0; j < ServerManager.Instance.dataByMonth.Count; j++)
            {
                ...
                if ((i + 1) == int.Parse(arrMonth[1]))
                {
                    ...
                    dayData_scrip.y = ServerManager.Instance.dataByMonth[j].month_StepCount;
                    dayData_scrip.posY = Mathf.Lerp(0, 632, Mathf.InverseLerp(0, top_heightSize, ServerManager.Instance.dataByMonth[j].month_StepCount));
                    dayData_scrip.LengthExtend();
