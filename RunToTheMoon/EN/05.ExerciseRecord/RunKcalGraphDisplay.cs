...
public class RunKcalGraphDisplay : MonoBehaviour
{
    public static RunKcalGraphDisplay instance { get; private set; }
    ...
    public void MonthInitalization()
    {
        ...
        DayDataDelete();
        _GraphTypeImge("dot");
        RunKcalGraphDrawLine.instance.GraphDrawLineMake();
    }
    ...
    public void DayDataInitialization(string _year)
    {
        ...
        { //가입 년도와 현재 년도가 다를 때 (2019 - 2020)
            //보여지는 년도와 올해 년도가 같으면(2021-2021)
    ...
    IEnumerator _DayDataInit(int _year, int _presentMonth, int _today, int _startDay)
    {
        ...
        {
            if (top_heightSize < ServerManager.Instance.dataByDate[max].today_Kcal)
            {
                top_heightSize = ServerManager.Instance.dataByDate[max].today_Kcal;
                biggestValue = ServerManager.Instance.dataByDate[max].today_Kcal;
            }
        }
        ...
        _one_third_Size.text = one_third_heightSize.ToString() + " Kcal";
        _two_third_Size.text = two_third_heightSize.ToString() + " Kcal";
        ...
        for (int i = _startDay; i < _today; i++)
        {
            ...
            if (_presentMonth == 1)
            {
                ...
                for (int j = 0; j < ServerManager.Instance.dataByDate.Count; j++)
                {
                    ...
                    {
                        dayData_scrip.y = ServerManager.Instance.dataByDate[j].today_Kcal; //서버에서 값을 들고오세욧1!!!! 일별 시간!! 배열로 들고오기!!! 
    ...
    IEnumerator _MonthDataInit(int _year, int _startMonth, int _endMonth)
    {
        ...
        {
            if (top_heightSize < ServerManager.Instance.dataByMonth[max].month_Kcal)
            {
                top_heightSize = ServerManager.Instance.dataByMonth[max].month_Kcal;
                biggestValue = ServerManager.Instance.dataByMonth[max].month_Kcal;
            }
        }
        top_heightSize += top_heightSize / 10;
        ...
        for (int i = _startMonth; i < _endMonth; i++)
        {
            for (int j = 0; j < ServerManager.Instance.dataByMonth.Count; j++)
            {
                ...
                if ((i + 1) == int.Parse(arrMonth[1]))
                {
                    ...
                    dayData_scrip.y = ServerManager.Instance.dataByMonth[j].month_Kcal;
                    dayData_scrip.posY = Mathf.Lerp(0, 632, Mathf.InverseLerp(0, top_heightSize, ServerManager.Instance.dataByMonth[j].month_Kcal));
                    dayData_scrip.LengthExtend();
