...
public class NickName_AppManager : MonoBehaviour
{
    ...
    IEnumerator _NickName_OverLap_Check()
    {
        ...
        {
            SoundFunction.Instance.ButtonClick_Sound();
            overlapText.text = "This is an available nickname.";
        }
        //중복체크를 하지 않았다.
        else if (specialState == true)
        {
            //닉네임을 저장할 수 없다.
            SoundFunction.Instance.Warning_Sound();
            overlapText.text = "Do not use special characters.";
        }
        else if (nullState == true)
        {
            SoundFunction.Instance.Warning_Sound();
            overlapText.text = "Please enter your nickname within 1 to 10 characters.";
        }
        else if(unavailableState == true)
        {
            SoundFunction.Instance.Warning_Sound();
            overlapText.text = "This nickname is not available.";

        }
        else if (overlapState == true)
        {
            SoundFunction.Instance.Warning_Sound();
            overlapText.text = "The nickname already exists.";
        }
        //둘중 하나라도 아닐 경우
        else
        {
            //닉네임을 저장할 수 없다.
            SoundFunction.Instance.Warning_Sound();
            overlapText.text = "Please set the nickname again.";
        }
    ...
    public void NickName_Save()
    {
        ...
        {
            //닉네임을 저장할 수 없다.
            overlapText.text = "Please double-check the nicknames.";
            SoundFunction.Instance.Warning_Sound();
        }
        //둘중 하나라도 아닐 경우
        else
