...
public class NickNameManager : MonoBehaviour
{
    ...
    IEnumerator _NickName_OverLap_Check()
    {
        ...
        {
            LocaleManager.Instance.GetStringLocale(availableNickName);
            //overlapText.text = "사용가능한 닉네임입니다.";
        }
        else if(curseState.Equals(true))
        {
            LocaleManager.Instance.GetStringLocale(usedSwearWords);
        ...
        {
            //닉네임을 저장할 수 없다.
            LocaleManager.Instance.GetStringLocale(usedSpecialWords);
            //overlapText.text = "특수문자를 사용했습니다. 다시 입력해주세요.";
        }
        else if (nullState == true)
        {
            //overlapText.text = "닉네임을 입력하세요";
            LocaleManager.Instance.GetStringLocale(enterTheNickName);
            nullState = false;
        }
        else if (overlapState == true)
        {
            LocaleManager.Instance.GetStringLocale(existNickName);
            //overlapText.text = "닉네임 중복입니다.";
        }
        //둘중 하나라도 아닐 경우
        else
        {
            //닉네임을 저장할 수 없다.
            LocaleManager.Instance.GetStringLocale(resetNickName);
    ...
    public void NickName_Save()
    {
        ...
        if(nicknameStr.Equals(nickname_field.text) && nicknameStr != "")
        {
            ...
            {
                //닉네임을 저장할 수 없다.
                LocaleManager.Instance.GetStringLocale(checkNickName);
