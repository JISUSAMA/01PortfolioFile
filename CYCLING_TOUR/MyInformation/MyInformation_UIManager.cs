...
public class MyInformation_UIManager : MonoBehaviour
{
    ...
    {
        profile_s = PlayerPrefs.GetInt("AT_Player_Profile");
        ...
        //내 기록
        todayKcalText.text = PlayerPrefs.GetFloat("AT_Record_TodayKcal").ToString() + " kcal";
        todayKmText.text = PlayerPrefs.GetFloat("AT_Record_TodayKm").ToString() + " km";
        totalKmText.text = PlayerPrefs.GetFloat("AT_Record_TotalKm").ToString() + " km";
        maxSpeedText.text = PlayerPrefs.GetFloat("AT_Record_MaxSpeed").ToString() + " m/s";
        ...
        if(PlayerPrefs.GetString("AT_Wear_GlovesKind") != "No")
        {
            ...
            gloves.sprite = Resources.Load<Sprite>("Install Item/Gloves/" + PlayerPrefs.GetString("AT_Wear_GlovesStyleName"));
        }
        else
        {
        ...
        if (PlayerPrefs.GetString("AT_Wear_HelmetKind") != "No")
        {
            ...
            helmatImg.sprite = Resources.Load<Sprite>("Install Item/Helmet/" + PlayerPrefs.GetString("AT_Wear_HelmetStyleName"));
        }
        else
        {
        ...
        pants.sprite = Resources.Load<Sprite>("Install Item/Pants/" + PlayerPrefs.GetString("AT_Wear_PantsStyleName"));
        ...
        shoes.sprite = Resources.Load<Sprite>("Install Item/Shoes/" + PlayerPrefs.GetString("AT_Wear_ShoesStyleName"));
        ...
        if (PlayerPrefs.GetString("AT_Player_Sex") == "Woman")
        {
            ...
            if (PlayerPrefs.GetString("AT_Wear_JacketStyleName") == "BasicNasi")
                jacket.sprite = Resources.Load<Sprite>("Install Item/Jacket/Woman/" + PlayerPrefs.GetString("AT_Wear_JacketStyleName"));
            else
                jacket.sprite = Resources.Load<Sprite>("Install Item/Jacket/" + PlayerPrefs.GetString("AT_Wear_JacketStyleName"));
            ...
            hair.sprite = Resources.Load<Sprite>("Install Item/Hair/Woman/" + PlayerPrefs.GetString("AT_Wear_HairStyleName"));
        ...
        else if (PlayerPrefs.GetString("AT_Player_Sex") == "Man")
        {
            ...
            Image jacket = jacketImg.transform.GetChild(0).transform.GetChild(0).GetComponent<Image>();
            if (PlayerPrefs.GetString("AT_Wear_JacketStyleName") == "BasicNasi")
                jacket.sprite = Resources.Load<Sprite>("Install Item/Jacket/Man/" + PlayerPrefs.GetString("AT_Wear_JacketStyleName"));
            else
                jacket.sprite = Resources.Load<Sprite>("Install Item/Jacket/" + PlayerPrefs.GetString("AT_Wear_JacketStyleName"));

            hairImg.transform.GetChild(0).gameObject.SetActive(true);
            Image hair = hairImg.transform.GetChild(0).transform.GetChild(0).GetComponent<Image>();
            hair.sprite = Resources.Load<Sprite>("Install Item/Hair/Man/" + PlayerPrefs.GetString("AT_Wear_HairStyleName"));
        }
    }
    ...
    {
        profileImg.sprite = Resources.Load<Sprite>("Profile/" + profile_s);
        PlayerPrefs.SetInt("AT_Player_Profile", profile_s);

        if(PlayerPrefs.GetString("AT_ProfileChange").Equals("MissionOk"))
            PlayerPrefs.SetString("AT_ProfileChange", "MissionOk");  //프로필 변경 여부
        else
            PlayerPrefs.SetString("AT_ProfileChange", "Yes");  //프로필 변경 여부

        ServerManager.Instance.UserProfile_Update();
    }

    //닉네임 초기화
    public void NickNameReset()
    {
        if (PlayerPrefs.GetInt("LocaleKey") == (int)LocaleManager.LanguageType.English) {
            overlapText.text = "Please enter a new name.";
        } else if (PlayerPrefs.GetInt("LocaleKey") == (int)LocaleManager.LanguageType.Korean) {
            overlapText.text = "새로운 이름을 입력해주세요.";        
        }
    }
    ...
    IEnumerator _NickNameChangeButtonOn()
    {
        ...
        {
            if (overlapText.text == "닉네임 성공" || overlapText.text == "Nickname change success.")
            {
                coin -= 1000;
                coinText.text = MyInformation_DataManager.instance.CommaText(coin);//.ToString();
                PlayerPrefs.SetString("AT_Player_NickName", nick_Field.text);
                PlayerPrefs.SetInt("AT_Player_Gold", coin);
                nickNameText.text = PlayerPrefs.GetString("AT_Player_NickName");
                nickNameChangePopup.SetActive(false);
                ServerManager.Instance.Update_Gold_Data(coin);
            }
        }
        else
        {
            ...
            if (PlayerPrefs.GetInt("LocaleKey") == (int)LocaleManager.LanguageType.English) {
                noticText.text = "Not enough coins." + "\n" + "Please use it again after acquiring the coin.";
            } else if (PlayerPrefs.GetInt("LocaleKey") == (int)LocaleManager.LanguageType.Korean) {
        ...
        for (int i = 0; i < ServerManager.Instance.myRank.Count; i++)
        {
            ...
            if (bestRank.Equals(99999))
            {
                if (int.Parse(rankText_s) < bestRank)
                {
                    ...
                    if (PlayerPrefs.GetInt("LocaleKey") == (int)LocaleManager.LanguageType.English) {
                        BestAllTimeText.text = rankMap_s + " " + rankCorse_s + "corse " + rankMode_s;
                    } else if (PlayerPrefs.GetInt("LocaleKey") == (int)LocaleManager.LanguageType.Korean) {
